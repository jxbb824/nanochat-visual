"""
Evaluate the vision-language model on the DocVQA benchmark:
https://huggingface.co/datasets/lmms-lab/DocVQA

We rely on tasks/docvqa.DocVQA to load the DocVQA subset via `datasets`
...
def generate_answer(model, tokenizer, question, visual_tokens, device, max_tokens=64, temperature=0.0, top_k=None):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VLM on DocVQA benchmark")
    parser.add_argument(
        "--vlm-tag",
        type=str,
    ...
    parser.add_argument(
        ...
        help="DocVQA split to evaluate on (we mainly use 'test')",
    )
    parser.add_argument(
        "--max-examples",
        type=int,
    ...
    with torch.no_grad(), autocast_ctx:
        for idx in range(n):
            conversation = ds[idx]
            image = conversation["docvqa_image"]
    ...
    if total > 0:
        acc = 100.0 * num_correct / total
        print(f"DocVQA accuracy ({args.split}): {num_correct}/{total} = {acc:.2f}%")
    else:
