"""
Evaluate the vision-language model on the MME benchmark:
https://huggingface.co/datasets/darkyarding/MME

We rely on tasks/mme.MME to load the test split via `datasets`,
and compute yes/no accuracy by comparing the model's first yes/no
prediction to the ground-truth `answer` field.
"""
...
def generate_answer(model, tokenizer, question, visual_tokens, device, max_tokens=8, temperature=0.0, top_k=None):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VLM on MME benchmark")
    ...
    if total > 0:
        acc = 100.0 * num_correct / total
        print(f"MME accuracy: {num_correct}/{total} = {acc:.2f}%")
    else:
