"""
Evaluate the vision-language model on the ChartQA benchmark:
https://huggingface.co/datasets/HuggingFaceM4/ChartQA

We rely on tasks/chartqa.ChartQA to load the dataset via `datasets`,
and evaluate accuracy by checking whether the ground-truth label string
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VLM on ChartQA benchmark")
    parser.add_argument(
        "--vlm-tag",
        type=str,
    ...
    parser.add_argument(
        ...
        help="ChartQA split to evaluate on",
    )
    parser.add_argument(
        "--max-examples",
        type=int,
    ...
    with torch.no_grad(), autocast_ctx:
        for idx in range(n):
            conversation = ds[idx]
            image = conversation["chartqa_image"]
    ...
    if total > 0:
        acc = 100.0 * num_correct / total
        print(f"ChartQA accuracy ({args.split}): {num_correct}/{total} = {acc:.2f}%")
    else:
